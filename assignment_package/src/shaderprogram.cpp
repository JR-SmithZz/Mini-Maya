...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1),attrUV(-1),attrPower(-1),attrAni(-1),
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrPower = context->glGetAttribLocation(prog, "vs_Pow");
    attrAni = context->glGetAttribLocation(prog, "vs_Ani");
 //std::cout<<attrPos<<" "<<attrNor<<" "<<attrCol<<std::endl;
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrPower != -1 && d.bindPower()) {
        context->glEnableVertexAttribArray(attrPower);
        context->glVertexAttribPointer(attrPower, 1, GL_FLOAT, false, 0, NULL);
    }

    if (attrAni != -1 && d.bindAni()) {
        context->glEnableVertexAttribArray(attrAni);
        context->glVertexAttribPointer(attrAni, 1, GL_FLOAT, false, 0, NULL);
    }
...
//std::cout<<"shader6"<<std::endl;
    ...
    if (attrPower != -1) context->glDisableVertexAttribArray(attrPower);
    if (attrAni != -1) context->glDisableVertexAttribArray(attrAni);
    context->printGLErrorLog();
    //std::cout<<"shader7"<<std::endl;
}

void ShaderProgram::drawS(Drawable &d)
{
        useMe();
