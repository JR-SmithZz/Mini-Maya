...
const vec3 dayColor = vec3(0,160,255)/255.0;
const vec3 nightColor = vec3(0,4,20)/255.0;
...
//    outColor = vec3(ndc * 0.5 + 0.5, 1);
    ...
    vec3 sunDir = vec3(0.0f, sin(phase), cos(phase));
    float sunSize = 30;
...
#define DUSK_THRESHOLD 0.5
   ...
   vec3 dayNightColor = mix(nightColor,dayColor,(0.5f*(sunDir.y+1.0f)));
   vec3 skyColor = mix(outColor,dayNightColor,(0.5f*(sunDir.y + abs(sunDir.y)+1.f)));
   outColor = skyColor;
   if(angle <= sunSize) {
       // Full center of sun
       if(angle < 7.5) {
            outColor = mix(skyColor,duskColor,(0.5f*(abs(sunDir.y)+1.f)));
            outColor = mix(outColor,sunColor,(0.5f*((sunDir.y)+1.f)));
       }
       // Corona of sun, mix with sky color
       else {
           vec3 roraColor = mix(skyColor,duskColor,(0.5f*(abs(sunDir.y)+1.f)));
           vec3 roraColor2 = mix(roraColor,sunColor,(0.5f*((sunDir.y)+1.f)));
           outColor = mix(roraColor2, skyColor, (angle - 7.5) / 22.5);
       }
   }
}
