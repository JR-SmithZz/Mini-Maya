...
vec4 lightDir = vec4(0,0,0,0);  // The direction of our virtual light, which is used to compute the shading of
...
void main()
{
    ...
    float time = u_Time * 0.001f;
    vec3 sunDir = normalize(vec3(0.1f, sin(time), cos(time)));
    vec4 lightDir = vec4(100 * sunDir, 0.0f);
